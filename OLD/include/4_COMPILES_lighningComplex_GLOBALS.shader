...
float calcFresnel(float cos_half_theta) {
...
float calcBeckmannInverse(float h_cos, float h_tan) {
    // Constants setup
...
float calcGeometricAttenuation(float l_cos, float h_cos, float cos_half_theta) {
...
float calcTorranceSparrow(float3 light_vector_normal) {
    ...
    //Numerator
    float fresnel = calcFresnel(cos_half_theta);
    float geometricAttenuation = calcGeometricAttenuation(l_cos, h_cos, cos_half_theta);
    float numerator = fresnel * geometricAttenuation;
    
    //Denominator
    float beckmannInverse = calcBeckmannInverse(h_cos, h_tan);
