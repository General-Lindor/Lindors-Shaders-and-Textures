...
s2half4 decode_normal(s2half4 value)
{
  ...
  s2half2 enc_normal = (2.0f * value.xy) - float2(1.0f, 1.0f);
  value = s2half4(
    enc_normal.x,
    enc_normal.y,
    sqrt(1.0f - dot(enc_normal, enc_normal)),
    value.z
  );
  ...
  value = s2half4(
    value.x,
    value.y,
    sqrt(1.0f - dot(value.xy, value.xy)),
    1.0f
  );
#elif NORMALFORMAT_DXN
	value     = (value - s2half4(0.5f, 0.5f, 0.5f, 0.0f)) * s2half4(2.0f, 2.0f, 2.0f, 1.0f);
  ...
  value *= s2half4(2.0f, 2.0f, 2.0f, 1.0f);
  value -= s2half4(1.0f, 1.0f, 1.0f, 0.0f);
