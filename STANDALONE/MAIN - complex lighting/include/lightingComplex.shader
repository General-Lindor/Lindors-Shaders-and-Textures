...
colors calcComposeColors(coldata I) {
    ...
    //colors
    #ifdef ENABLE_VERTEX_LIGHTING
        //Composition Model: Component based MAX
        ...
        //Composition Model: formula
        // float3 univec = float3(1.0f, 1.0f, 1.0f);
        // flooat3 rbg_base = univec - ((univec - col_mix) * (univec - col_mix_hero) * (univec - col_mix_vertex));
        
        //Composition Model: Add, then normalize with max Vector Length als lightness
        // float3 colorSum = col_mix + col_mix_hero + col_mix_vertex;
        // float len_std    = sqrt(dot(col_mix, col_mix));
        // float len_hero   = sqrt(dot(col_mix_hero, col_mix_hero));
        // float len_vertex = sqrt(dot(col_mix_vertex, col_mix_vertex));
        // float len_sum    = sqrt(dot(colorSum, colorSum));
        // float len_max    = max(len_std, max(len_hero, len_vertex));
