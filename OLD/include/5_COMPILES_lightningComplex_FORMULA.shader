...
// float3 composeVectors2(float3 args[2]) {
    // float3 first = args[0];
    // #ifdef SMOOTH_COMPOSE
...
// float composeFloats(int count, float args[count]) {
    // float3 first = args[0];
    // #ifdef SMOOTH_COMPOSE
...
#ifdef SPECULAR_GLOW
    #ifdef ENABLE_VERTEX_LIGHTING
                // float3 rgb_base = composeVectors(4, [col_amb, col_diff, col_amb_vertex, col_diff_vertex]);
        #ifdef SMOOTH_COMPOSE
...
// colors calcComposeColors(coldata I) {
    ...
    // // setup components
        // // vectors
        // float3 col_tex_base = saturate(I.texBase.xyz);
        // float3 col_tex_glow = saturate(I.texGlow.xyz);
        
        // float3 col_amb  = saturate(I.ambient * I.shadow * I.light_col_amb * col_tex_base);
        // float3 col_diff = saturate(I.diffuse * I.shadow * I.light_col_diff * col_tex_base);
        // float3 col_spec = saturate(I.specular * I.shadow * I.light_col_diff * col_tex_base);
        // #ifdef ENABLE_VERTEX_LIGHTING
            // float3 col_amb_vertex  = saturate(I.ambient * I.light_col_hero * col_tex_base);
            // float3 col_diff_vertex = saturate(I.diffuse_vertex * I.light_col_vertex * col_tex_base);
            // float3 col_spec_vertex = saturate(I.specular_vertex * I.light_col_vertex * col_tex_base);
        // #endif
        
        // // floats
        // float alpha_tex_base = saturate(I.texBase.w);
        // float alpha_tex_glow = saturate(I.texGlow.w);
        
        // float alpha_spec = saturate(dot(gamma_correction, col_spec));
        // #ifdef ENABLE_VERTEX_LIGHTING
            // float alpha_spec_vertex = saturate(dot(gamma_correction, col_spec_vertex));
        // #endif
    ...
    // // compose
        ...
        // #ifdef SPECULAR_GLOW
            // #ifdef ENABLE_VERTEX_LIGHTING
                // float3 rgb_base = composeVectors(4, [col_amb, col_diff, col_amb_vertex, col_diff_vertex]);
                // float3 rgb_glow = composeVectors(3, [col_tex_glow, col_spec, col_spec_vertex]);
            // #else
                // float3 rgb_base = composeVectors(2, [col_amb, col_diff]);
                // float3 rgb_glow = composeVectors(2, [col_spec, col_tex_glow]);
            // #endif
        // #else
            // #ifdef ENABLE_VERTEX_LIGHTING
                // float3 rgb_base = composeVectors(6, [col_amb, col_diff, col_spec, col_amb_vertex, col_diff_vertex, col_spec_vertex]);
            // #else
                // float3 rgb_base = composeVectors(3, [col_amb, col_diff, col_spec]);
            // #endif
            // float3 rgb_glow = col_tex_glow;
        // #endif
        
        // // transparencies
        // float alpha_base = alpha_tex_base;
        // #ifdef SPECULAR_GLOW
            // #ifdef ENABLE_VERTEX_LIGHTING
                // float alpha_glow = composeFloats(3, [alpha_tex_glow, alpha_spec, alpha_spec_vertex]);
            // #else
                // float alpha_glow = composeFloats(2, [alpha_tex_glow, alpha_spec)]);
            // #endif
        // #else
            // float alpha_glow = alpha_tex_glow;
        // #endif
