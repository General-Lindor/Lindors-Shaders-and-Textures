...
lightingLevels calcLightingLevels(complexLightingData I) {
    ...
    //Ambient
    float out_ambient = lerp(0.25f, 0.5f, context.ALBEDO * context.C_COS);
...
colors calcComposeColors(coldata I) {
    ...
    //setup components
    float3 col_amb  = saturate(abs(I.ambient * I.light_col_amb * I.texBase.xyz));
    ...
    #ifdef ENABLE_VERTEX_LIGHTING
        float3 col_amb_hero   = saturate(abs(I.ambient * I.light_col_hero * I.texBase.xyz));
        float3 col_amb_vertex = saturate(abs(I.ambient * I.light_col_vertex * I.texBase.xyz));
        #ifdef VERTEX_LIGHTING_NRM
    ...
    #ifdef ENABLE_VERTEX_LIGHTING
        float3 rgb_hero = (univec - col_amb_hero);
        #ifdef VERTEX_LIGHTING_NRM
    ...
    #ifdef ENABLE_VERTEX_LIGHTING
        float3 rgb_vertex = (univec - col_amb_vertex);
        #ifdef VERTEX_LIGHTING_NRM
