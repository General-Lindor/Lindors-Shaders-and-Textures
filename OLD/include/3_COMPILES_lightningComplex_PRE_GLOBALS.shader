...
float calcFresnel(float cos_half_theta, float refr_idx) {
    ...
    // float cos_half_theta = sqrt((cos_theta + 1.0f) * 0.5f);
    
    //The refractive index of air approximately equals 1.0f
    float r_theta = (1.0f - refr_idx) / (1.0f + refr_idx);
    r_theta *= r_theta;
    float fresnel = r_theta + ((1.0f - r_theta) * pow((1.0f - cos_half_theta), 5));
    return fresnel;
};

// Inverse of the Beckmann Distribution
float calcBeckmannInverse(float h_cos, float h_tan, float roughness) {
    // Constants setup
    float alpha = (((1.0f - (2.0f * roughness)) / sqrt(const_pi)) + roughness) / (1.0f - roughness);
    
    // calcuation
    float n = (h_tan / alpha);
    n *= n;
    float d = alpha * h_cos * h_cos;
    d *= d;
    float beckmannInverse = const_pi * (pow(const_e, n)) * d;
...
float calcGeometricAttenuation(float c_cos, float l_cos, float h_cos, float cos_half_theta) {
    float factor = (2.0f * h_cos) / cos_half_theta;
...
float calcTorranceSparrow(float3 camera_vector_normal, float3 light_vector_normal, float3 normal, float roughness, float refr_idx) {
    //setup
        ...
        float h_dot = dot(normal, half_vector_normal);
        float3 h_cross = cross(normal, half_vector_normal);
        ...
        //float cos_half_theta = dot(half_vector_normal, light_vector_normal);
        float cos_half_theta = dot(half_vector_normal, camera_vector_normal);
        
        
    //Numerator
    float fresnel = calcFresnel(cos_half_theta, refr_idx);
    float geometricAttenuation = calcGeometricAttenuation(c_cos, l_cos, h_cos, cos_half_theta);
    float numerator = fresnel * geometricAttenuation;
    
    //Denominator
    float beckmannInverse = calcBeckmannInverse(h_cos, h_tan, roughness);
    float denominator = 4.0f * beckmannInverse * c_cos * l_cos;
    
    //out
    float specular = saturate((numerator / denominator));
    return specular;
};
...
lightingLevels calcLightingLevels(complexLightingData I) {
    ...
    // extract
    float3 normal = I.nrm;
    float3 camera_vector_normal = I.c_dir_obj_nrm;
    ...
    // Always the same
    float lambertian = calcLambertian(camera_vector_normal, normal);
    ...
    // Diffuse
    #ifdef OREN_NAYAR
        float out_diffuse = calcOrenNayar(camera_vector_normal, light_vector_normal, normal, roughness, albedo);
        #ifdef ENABLE_VERTEX_LIGHTING
            #ifdef VERTEX_LIGHTING_NRM
                float out_diffuse_vertex = calcOrenNayar(camera_vector_normal, light_vector_vertex_normal, normal, roughness, albedo);
            #else
    ...
    // Specular
    #ifdef TORRANCE_SPARROW
        float out_specular = calcTorranceSparrow(camera_vector_normal, light_vector_normal, normal, roughness, refr_idx);
        #ifdef ENABLE_VERTEX_LIGHTING
            #ifdef VERTEX_LIGHTING_NRM
                float out_specular_vertex = calcTorranceSparrow(camera_vector_normal, light_vector_vertex_normal, normal, roughness, refr_idx);
            #else
                float out_specular_vertex = lambertian;
            #endif
        #endif
    #elif defined(PHONG)
        float out_specular = calcPhong(camera_vector_normal, light_vector_normal, normal, roughness);
        #ifdef ENABLE_VERTEX_LIGHTING
            #ifdef VERTEX_LIGHTING_NRM
                float out_specular_vertex = calcPhong(camera_vector_normal, light_vector_vertex_normal, normal, roughness);
            #else
                float out_specular_vertex = lambertian;
            #endif
        #endif
