...
float calcFresnel(float c_h_cos) {
...
float calcBeckmann(float h_cos) {
    //Constants setup
    ...
    //out
    float beckmann = 1.0f / beckmannInverse;
    beckmann = saturate(beckmann);
    return beckmann;
};

//Geometric Attenuation
float calcGeometricAttenuation(float l_cos, float h_cos, float c_h_cos) {
    //calcuation
...
float calcTorranceSparrow(float3 light_vector_normal) {
    ...
    //Numerator
    float fresnel = calcFresnel(c_h_cos);
    float geometricAttenuation = calcGeometricAttenuation(l_cos, h_cos, c_h_cos);
    float beckmann = calcBeckmann(h_cos);
    float numerator = fresnel * geometricAttenuation * beckmann;
    
    //Denominator
    float denominator = 4.0f * c_cos * l_cos;
...
lightingLevels calcLightingLevels(complexLightingData I) {
    ...
    //Specular
        ...
        float out_specular = calcPhong(light_vector_normal);
            ...
            float out_specular_vertex = lambertian;
        #endif
    #elif defined(PHONG_BLINN)
        float out_specular = calcPhongBlinn(light_vector_normal);
        #ifdef VERTEX_LIGHTING_NRM
            float out_specular_vertex = calcPhongBlinn(light_vector_vertex_normal);
        #else
            float out_specular_vertex = lambertian;
        #endif
    #else
