...
lightingLevels calcLightingLevels(complexLightingData I) {
    ...
    //Fresnel (Schlick's Approximation)
        float r_theta = (1.0f - refr_idx) / (1.0f + refr_idx);
        r_theta *= r_theta;
        float fresnel = r_theta + ((1.0f - r_theta) * pow((1.0f - c_cos), 5));
#endif
    ...
    //Specular Model: Torrance-Sparrow / Cook-Torrance
        ...
        //Denominator
            //Beckmann Distribution (note that b_d actually calculates the inverse of the beckmann distribution)
                float n = (h_tan / roughness);
                n *= n;
                float d = roughness * h_cos * h_cos;
                d *= d;
                float b_d = ((pow(const_e, n)) * d);
            
            float denominator = 4.0f * const_pi * b_d * c_cos * l_cos;
    ...
    //Specular Model: Torrance-Sparrow / Cook-Torrance - vertex
        ...
        //Denominator
            //Beckmann Distribution (note that b_d actually calculates the inverse of the beckmann distribution)
                ...
                d_vertex *= d_vertex;
                float b_d_vertex = ((pow(const_e, n_vertex)) * d_vertex);
            
            float denominator_vertex = 4.0f * const_pi * b_d_vertex * c_cos * v_cos;
        
        float out_specular_vertex = numerator_vertex / denominator_vertex;
    #else
        //vertex diffuse: Just Fresnel
        float out_specular_vertex = pow((fresnel), (roughness * 40.0f));
    #endif
#elif defined(PHONG)
    //Specular Model: Phong with Fresnel
        float out_specular = pow((dot(c_dir_obj_nrm.xyz, ((2.0f * l_cos * nrm.xyz) - l_dir_obj_nrm.xyz)) * fresnel), (roughness * 40.0f));
    #ifdef VERTEX_LIGHTING_NRM
    //Specular Model: Phong with Fresnel
        float out_specular_vertex = pow((dot(c_dir_obj_nrm.xyz, ((2.0f * v_cos * nrm.xyz) - nrm_vertex.xyz)) * fresnel), (roughness * 40.0f));
    #else
        //vertex diffuse: Just Fresnel
        float out_specular_vertex = pow((fresnel), (roughness * 40.0f));
    #endif
    ...
    //Specular Model: Just Fresnel
        float out_specular = pow((fresnel), (roughness * 40.0f));
